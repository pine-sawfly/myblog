...
![1583292290939-2db52ed1-6f40-488d-be02-569b83b66a63.png](./img/557519_sochjnA5LHEVTmKN/1583292290939-2db52ed1-6f40-488d-be02-569b83b66a63-258328.png)
...
双击并运行360zip.exe，程序仍然可以正常运行，图标由“熊猫烧香”变为“程序原图标”，并且文件夹中多出来Desktop\_.ini的文件，打开记录着系统中毒时间：2020-3-4
...
浏览筛选项就会发现，它在C盘的根目录下创建了setup.exe、autorun.inf和Desktop*.ini，由于在前面我们发现病毒会隐藏文件，我们有理由详细，它创建的setup.exe、autorun.inf和Desktop*.ini也是具有隐藏属性的（事实证明也是如此）
...
>
> 5.创建autorun.inf、Desktop\_.ini、setup.exe，这些文件属性均为隐藏
...
mov edx,0x408798是将Desktop\_.ini的地址（00408798）赋给EDX
...
filename的参数保存在eax中，eax保存的正好就是路径的地址：C:\Documents and Settings\Administrator\桌面\Desktop*.ini，也就是说call <jmp.&kernel32.FindFirstFileA> 是要查找当前目录下的Desktop*.ini文件是否存在
...
test al,al是进行验证Desktop\_.ini是否存在，如果存在，则跳转不发生执行setFileAttributesA来设置文件的属性为正常（push 0x80），停止1ms，最后将文件删除。
...
> DWORD \_\_fastcall sub_40521C(int a1, LPSTR \*a2)
>
> {
>
>  LPSTR \*v2; // edi
...
int \_\_usercall sub_4053AC@<eax>(int \*a1@<eax>)
